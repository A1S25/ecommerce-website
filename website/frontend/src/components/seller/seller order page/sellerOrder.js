...
export default function SellerOrder() {
    ...
    const columns = [
        ...
        { field: "amount", headerName: "Amount", type: "number", minWidth: 270, flex: 0.5, },
        {
            ...
            renderCell: (params) => {
                return (
                    <Fragment>
                        <Link to={`/seller/order/${params.getValue(params.id, "id")}`}> <EditIcon /> </Link>
    ...
    orders && orders.forEach((item) => {
        rows.push({ id: item._id, itemsQty: item.orderItems.length, amount: item.totalPrice, status: item.orderStatus, });
    });
